...
async function getEmbeddingConfig() {
  const providerType =
    (await getConfig("system.EMBEDDING_PROVIDER")) ||
    process.env.EMBEDDING_PROVIDER ||
    DEFAULT_PROVIDER;

  const defaults =
    PROVIDER_DEFAULTS[providerType] || PROVIDER_DEFAULTS[DEFAULT_PROVIDER];

  return {
    providerType,
    model:
      (await getConfig("system.EMBEDDING_MODEL")) ||
      process.env.EMBEDDING_MODEL ||
      defaults.model,
    geminiApiKey:
      (await getConfig("system.GEMINI_API_KEY")) ||
      process.env.GEMINI_API_KEY ||
      "",
    openaiApiKey:
      (await getConfig("system.OPENAI_API_KEY")) ||
      process.env.OPENAI_API_KEY ||
      "",
    jinaApiKey:
      (await getConfig("system.JINA_API_KEY")) ||
      process.env.JINA_API_KEY ||
      "",
    ollamaBaseUrl:
      (await getConfig("system.OLLAMA_BASE_URL")) ||
...
async function embedWithOpenAI(modelName, apiKey, texts) {
  ...
  for (let i = 0; i < texts.length; i += CHUNK) {
    ...
    if (!resp.ok) {
      ...
      throw new Error(
        `OpenAI embedding error ${resp.status}: ${err.slice(0, 300)}`,
      );
    }
...
async function embedWithJina(modelName, apiKey, texts) {
  ...
  for (let i = 0; i < texts.length; i += CHUNK) {
    ...
    if (!resp.ok) {
      const err = await resp.text();
      throw new Error(
        `Jina embedding error ${resp.status}: ${err.slice(0, 300)}`,
      );
    }
