...
const FIXED_PROVIDERS = [
  ...
  { value: 'gemini',     label: 'Gemini',     capabilities: ['embeddings'], requires_api_key: true },
...
async function getProviderDefinitions(capability) {
  ...
  function addProvider(provider) {
    ...
    if (existing) {
      existing.capabilities = Array.from(new Set([...existing.capabilities, ...provider.capabilities]))
    } else {
      byValue.set(provider.value, { ...provider, capabilities: [...provider.capabilities] })
    }
  }
  ...
  return Array.from(byValue.values()).filter(provider => !capability || provider.capabilities.includes(capability))
}
